...
export default function FatigueIndexChart() {
  ...
  useEffect(() => {
    ...
    console.log(`[FatigueIndexInjuryChart] Players out per game — Team: ${selectedTeam}`);
    chartData.forEach((d) => {
      console.log(`  Game ${d.game_number} (${d.game_date}): ${d.players_out} players out`);
    });
    console.log(`[FatigueIndexInjuryChart] Summary — min: ${Math.min(...chartData.map((x) => x.players_out))}, max: ${Math.max(...chartData.map((x) => x.players_out))}`);
  ...
  useEffect(() => {
    ...
    if (pointsWithFatigue.length === 0) {
      ...
      g.append("text")
        ...
        .text("Game number");
      const emptyYLabelX = 24;
      const emptyYLabelY = MARGIN.top + innerHeight / 2;
      svg.append("text")
        .attr("x", emptyYLabelX)
        .attr("y", emptyYLabelY)
        .attr("text-anchor", "middle")
        .attr("transform", `rotate(-90, ${emptyYLabelX}, ${emptyYLabelY})`)
        .attr("fill", CHART_THEME.textMuted)
        .attr("font-size", "12px")
        .attr("font-family", CHART_THEME.fontFamily)
        .text("Fatigue index");
  ...
  /** Y-axis label: append to SVG so it stays in the left margin, outside the chart area */
  const yLabelX = 24;
  const yLabelY = MARGIN.top + (HEIGHT - MARGIN.top - MARGIN.bottom) / 2;
  svg.append("text")
    .attr("x", yLabelX)
    .attr("y", yLabelY)
    .attr("text-anchor", "middle")
